...
class SwinMono(nn.Module):
    def __init__(self, max_depth=10.0, is_train=False, version='base07', frozen_stages=-1):
        super().__init__()
        ...
        if is_train:            
            ckpt_path = './ckpt/swin_base_patch4_window7_224_22k.pth'
            try:
                self.encoder.init_weights(pretrained=ckpt_path)
            except:
                print("load swin transformer failed...")
               

        #base swin
        channels_in = [1024, 512, 256]
        channels_out = 128
